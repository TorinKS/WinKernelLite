...
/**
 * @brief Advanced corruption pattern tests targeting specific corruption scenarios
 * 
 ...
 * - Registry path co        test.data = {L'T', L'E', L'S', L'T', L'\0', L'\0'};
        test.dataSize = static_cast<ULONG>(test.data.size() * sizeof(WCHAR));
        stringTests.push_back(test);tenation errors
 * - Memory alignment issues
 * - Buffer boundary violations
 * - String termination problems
 */
class RegistryCorruptionPatternsTest : public ::testing::Test {
protected:
    HANDLE testKeyHandle;
    std::wstring testKeyPath;
    
    void SetUp() override {
        testKeyPath = L"SOFTWARE\\WinKernelLite\\PatternsTest";
 ...
 */
TEST_F(RegistryCorruptionPatternsTest, ReproduceObservedCorruption) {
 ...
 */
TEST_F(RegistryCorruptionPatternsTest, TestBufferBoundaryConditions) {
 ...
 */
TEST_F(RegistryCorruptionPatternsTest, TestMemoryAlignmentIssues) {
 ...
 */
TEST_F(RegistryCorruptionPatternsTest, TestStringConcatenationCorruption) {
 ...
 */
TEST_F(RegistryCorruptionPatternsTest, TestStringTerminationIssues) {
