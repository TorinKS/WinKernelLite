...
void CreateMemoryLeak() {
    ...
    printf("Created a deliberate memory leak of 1024 bytes at address %p\n", leakedMemory);
}
...
int main(void)
{
	...
	DebugSetComponentMask(DEBUG_COMPONENT_HEAP | DEBUG_COMPONENT_TRACKING);
	DebugEnableFileOutput(TRUE, "DevicesList_debug.log");
     ...
     */
    CreateMemoryLeak();  // Temporarily enabled to test leak detection
    ...
    // Call the library function (writes to log files)
    PrintMemoryLeaks();
    ...
    if (state) {
        ...
        printf("  Current bytes allocated: %d\n", (int)state->CurrentBytesAllocated);
        printf("  Double-free attempts: %d\n", (int)state->DoubleFreeCount);
        
        if (state->CurrentBytesAllocated > 0) {
            printf("WARNING: %d bytes still allocated (potential memory leaks!)\n", 
                   (int)state->CurrentBytesAllocated);
            printf("Check the log files for detailed leak information.\n");
        } else {
